...
?>
    ...
    <main class="flex-1 p-8 overflow-y-auto">
        ...
        <div class="bg-black/30 backdrop-blur-sm p-6 md:p-8 rounded-xl shadow-2xl border border-white/10 max-w-2xl mx-auto">
            ...
            <form action="index.php?page=pelanggan&action=edit&id=<?= urlencode($data['no_ktp']) ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    <?php $fotoLabel = empty($data['foto_sim']) ? 'Foto SIM <span class="text-amber-300">(wajib)</span>' : 'Foto SIM (opsional)'; ?>
                    <label for="foto_sim" class="block mb-2 text-sm font-medium text-text-primary-dark"><?= $fotoLabel ?></label>
                    <input type="file" id="foto_sim" name="foto_sim" accept="image/*" class="w-full text-sm text-white/80" <?= empty($data['foto_sim']) ? 'required' : '' ?> >
                    <?php if (!empty($data['foto_sim'])): ?>
                        <p class="text-text-secondary-dark text-sm mt-2">Foto SIM saat ini: <a href="<?= htmlspecialchars($data['foto_sim']) ?>" target="_blank" class="underline">Lihat</a></p>
                    <?php endif; ?>
                    <div id="foto_preview" class="mt-2">
                        <?php if (!empty($data['foto_sim'])): ?>
                            <img src="<?= htmlspecialchars($data['foto_sim']) ?>" style="max-width:200px;max-height:150px" class="rounded-md border" />
                        <?php endif; ?>
                    </div>
